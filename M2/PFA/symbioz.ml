...
let insert_at_random elt list = insert_at_pos elt list (Random.int (List.length list));;
...
module Symbioz:PLANETE =
  struct
    ...
    let rec at_pos get_a_pos pos list = match list with
      | [] -> []
...
adulte ou non, on initialise donc aussi le second champs *)
    ...
    let vieillir individu = 
      if snd individu.age > 1 then (* on reduit l'esperance de vie dans tous les cas *)
	Some {individu with age = (get_age individu, snd individu.age - 1)}
      else if get_age individu = Bebe then (* Sinon on change d'age *)
